...
function GuideCard({ r }) {
  ...
  return r.external ? (
    <a href={r.href} target="_blank" rel="noopener noreferrer" className={`${cardCls} transition-colors`}>{body}</a>
  ) : (
    <Link to={r.href} className={`${cardCls} transition-colors`}>{body}</Link>
  );
}
